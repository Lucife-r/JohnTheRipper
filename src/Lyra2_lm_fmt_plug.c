...
john_register_one(&fmt_lyra2_lm);
#else
...
static int alloc;
static struct lyra2_lm_allocation *allocated;
...
static void set_salt(void *salt)
{
	...
	for(i=0;i<threads;i++)
	{
		pthread_barrier_init(&barrier[i],NULL,saved_salt.nThreads);
...
#pragma omp parallel for
		for (i = 0; i < MIN(remaining,threads); i++) {
			...
			int hash_instance = (thread+prev)/saved_salt.nThreads;
			...
			LYRA2_LM
			    ...
			    saved_salt.salt_length, saved_salt.t_cost, saved_salt.m_cost,
		            saved_salt.nThreads, threadNumber, &allocated[hash_instance%threads], &barrier[hash_instance%threads]);
		}
	}
	return count;
}
