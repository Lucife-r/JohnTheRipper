...
int ARGON2i_OPT(void *out, size_t outlen, const void *in, size_t inlen, const void *salt, size_t saltlen,
 ...
 {
	return Argon2iOpt((uint8_t*)out, (uint32_t)outlen, (const uint8_t*)in, (uint32_t)inlen, (const uint8_t*)salt, (uint32_t)saltlen, NULL, 0, NULL, 0, (uint32_t)t_cost, (uint32_t)m_cost, lanes, memory);
 }
