...
*/
#ifndef __OPENCL_BLAKE2_H__
#define __OPENCL_BLAKE2_H__
...
typedef struct __blake2s_param {
	...
	// uchar  reserved[0];
	uchar salt[BLAKE2B_SALTBYTES];	// 24
...
typedef struct __blake2sp_state {
	blake2s_state S[8][1];
	blake2s_state R[1];
	uchar buf[8 * BLAKE2S_BLOCKBYTES];
	size_t buflen;
} blake2sp_state;
...
typedef struct __blake2bp_state {
	blake2b_state S[4][1];
	blake2b_state R[1];
	uchar buf[4 * BLAKE2B_BLOCKBYTES];
	size_t buflen;
} blake2bp_state;

/*
  static inline int blake2( uchar *out, const void *in, const void *key, const uchar outlen, const ulong inlen, uchar keylen )
  {
...
static inline int blake2b_param_set_inner_length(blake2b_param * P,
    const uchar inner_length)
{
	P->inner_length = inner_length;
	return 0;
}

static inline int blake2b_param_set_salt(blake2b_param * P, const uchar * salt)
{
...
static int blake2b_compress(blake2b_state * S,
    const uchar block[BLAKE2B_BLOCKBYTES])
{
	ulong m[16];
	ulong v[16];
	int i;
...
/* inlen now in bytes */
int blake2b_update(blake2b_state * S, const uchar * in, ulong inlen)
{
	while (inlen > 0) {
...
int blake2b_init_key(blake2b_state * S, const uchar outlen, const void *key,
    const uchar keylen)
{
	blake2b_param P[1];
...
/* Is this correct? */
int blake2b_final(blake2b_state * S, uchar * out, uchar outlen)
{
	...
	for (i = 0; i < 8; ++i)	/* Output full hash to temp buffer */
		((ulong *) buffer)[i] = S->h[i];

	memcpy(out, buffer, outlen);
	return 0;
}

int blake2b_final_g(blake2b_state * S, __global uchar * out, uchar outlen)
{
	uchar buffer[BLAKE2B_OUTBYTES];
	int i;
	...
	memcpy_g(out, buffer, outlen);
	return 0;
}

/* inlen, at least, should be ulong. Others can be size_t. */
int blake2b(uchar * out, const void *in, const void *key, const uchar outlen,
    const ulong inlen, uchar keylen)
{
	blake2b_state S[1];
	...
	blake2b_update(S, (uchar *) in, inlen);
