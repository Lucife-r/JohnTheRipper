...
static void ComputeBlock_pgg(ulong2 *state, __global ulong2 *ref_block_ptr, __global ulong2 *next_block_ptr)
{
	...
	#define A 0
	ulong8 v1=(ulong8) (state[0+A], state[8+A], state[16+A], state[24+A]);
	#define A 1
	ulong8 v2=(ulong8) (state[0+A], state[8+A], state[16+A], state[24+A]);
	#define A 2
	ulong8 v3=(ulong8) (state[0+A], state[8+A], state[16+A], state[24+A]);
	#define A 3
	ulong8 v4=(ulong8) (state[0+A], state[8+A], state[16+A], state[24+A]);
	#define A 4
	ulong8 v5=(ulong8) (state[0+A], state[8+A], state[16+A], state[24+A]);
	#define A 5
	ulong8 v6=(ulong8) (state[0+A], state[8+A], state[16+A], state[24+A]);
	#define A 6
	ulong8 v7=(ulong8) (state[0+A], state[8+A], state[16+A], state[24+A]);
	#define A 7
	ulong8 v8=(ulong8) (state[0+A], state[8+A], state[16+A], state[24+A]);
...
static void FillSegment(const scheme_info_t* info, const position_info_t position)
{
	...
	if(0 == pass && 0 == slice) // First pass; first slice
	{
		...
		bi = (lane * segment_length + 1) * BLOCK_SIZE/16;//<bi> -- temporary variable for loading previous block
		for (i = 0; i < 64; i++)
		{
	...
	else
	{
		...
		uint bi = ((prev_slice * lanes + lane + 1) * segment_length - 1) * BLOCK_SIZE/16;//<bi> -- temporary variable for loading previous block
		for (i = 0; i < 64; i++)
		{
			prev_block[i] = memory[MAP(bi+i)];
		}
	...
	next_block_offset = ((slice*lanes + lane)*segment_length + start)*BLOCK_SIZE/16;
...
)
{
	...
	Argon2i(out, outlen, passwd, inlen, nonce, noncelen, 0, 0, 0, 0, t_cost, m_cost, lanes, memory);
}
