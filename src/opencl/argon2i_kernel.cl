...
static void ComputeBlock_pgg(ulong2 *state, __global ulong *ref_block_ptr, __global ulong *next_block_ptr)
{
	ulong2 ref_block[64];
	uchar i;
	...
	for (i = 0; i < 128; i++)
	{
		((ulong*)ref_block)[i] = ref_block_ptr[MAP(i)];
	}
	...
	for (i = 0; i< 128; i++)
	{
		next_block_ptr[MAP(i)]=((ulong*)state)[i];
	}
}
